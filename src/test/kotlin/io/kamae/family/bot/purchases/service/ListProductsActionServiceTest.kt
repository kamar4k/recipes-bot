...
class ListProductsActionServiceTest : AbstractTest() {
        ...
        val expected = TelegramActionResult(
            TelegramResponse(
                ...
                OneColumnKeyboard(
                    listOf(
                        TelegramButton(
                            "$TEST_PRODUCT_NAME | Кол-во: $TEST_PRODUCT_CURR_QUANTITY | Прогноз: $TEST_PRODUCT_PREDICATION_STR | $MINUS_EMOJI",
                            "/reduce-product $TEST_PRODUCT_ID $TEST_PRODUCT_CURR_QUANTITY"
                        ),
                        TelegramButton(
                            "$TEST_PRODUCT_NAME_ANOTHER | Кол-во: $TEST_PRODUCT_CURR_QUANTITY_ANOTHER | Прогноз: $TEST_PRODUCT_PREDICATION_ANOTHER_STR | $PLUS_EMOJI",
    ...
    fun executeAction_integrationError() {
        ...
        val error = assertThrows<IllegalStateException> { listProductsActionService.executeAction(formAction()) }
